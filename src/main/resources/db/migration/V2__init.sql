...
update c_assemblies set ac_name=REPLACE (`ac_name`, '(SC)', '')
WHERE st_name = 'DELHI';
update c_constituency cc inner join c_assemblies cp on REPLACE(REPLACE (cp.`ac_name`, ' ', ''), '.', '') LIKE CONCAT('%', REPLACE (cc.`name`, ' ', ''), '%') 
set cc.`polygon_id`= cp.`objectid`;